...
mod tests {
    ...
    fn request(peer: &str, forwarded_for: Option<&str>) -> Request<Body> {
        ...
        request.extensions_mut().insert(ConnectInfo(
            peer.parse::<SocketAddr>().expect("测试对端地址应当有效"),
        ));
        if let Some(forwarded_for) = forwarded_for {
            request.headers_mut().insert(
                X_FORWARDED_FOR,
                forwarded_for.parse().expect("测试代理头应当有效"),
            );
        }
        request
    }
    ...
    #[tokio::test]
    async fn untrusted_peer_cannot_spoof_forwarded_ip() {
        let limiter = OrderCreationRateLimiter::new(&["10.0.0.0/8".parse().unwrap()]);
        let request = request("198.51.100.7:443", Some("203.0.113.99"));
    ...
    #[tokio::test]
    async fn trusted_proxy_chain_stops_at_first_untrusted_client() {
        let limiter = OrderCreationRateLimiter::new(&[
            "10.0.0.0/8".parse().unwrap(),
            "192.168.0.0/16".parse().unwrap(),
        ]);
        let request = request(
            "10.0.0.2:443",
            Some("203.0.113.99, 198.51.100.7, 192.168.1.2"),
        );
    ...
    #[tokio::test]
    async fn sixth_order_creation_is_rejected_with_json_429_and_retry_after() {
        let limiter = OrderCreationRateLimiter::new(&[]);
        let app = Router::new()
            .route("/", get(|| async { StatusCode::NO_CONTENT }))
            .layer(middleware::from_fn_with_state(
                limiter,
                enforce_order_creation_limit,
            ));

        for _ in 0..ORDER_CREATION_LIMIT_MAX_REQUESTS {
            let response = app
                .clone()
                .oneshot(request("198.51.100.7:443", None))
                .await
                .unwrap();
        ...
        let response = app
            .oneshot(request("198.51.100.8:443", None))
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::NO_CONTENT);
    }
